...
void* mm_malloc(size_t size) {
  ...
  block_info* ptr_next_block = NULL; //Instantiate struct for a new block in case the block size returned is not exactly equal to our request
  size_t block_size;
  ...
  // If the free list does not have a big enough free block, request more heap space and assign it to our free block pointer
  if(search_free_list(req_size) == NULL){
    request_more_space(req_size);
  }
  ptr_free_block = search_free_list(req_size);

  // Extract relevant information from the free block's header (size and preceding_used bit) and store for later
  block_size = SIZE(ptr_free_block->size_and_tags);
  preceding_block_use_tag = TAG_PRECEDING_USED & (ptr_free_block->size_and_tags);

  // Flip the valid bit to 1 to indicate that this block has been allocated
  ptr_free_block->size_and_tags = ptr_free_block->size_and_tags | TAG_USED;

  // Remove this block from the free list.
  remove_free_block(ptr_free_block);

  // If this free block has enough space left over after the allocation to create another free block...
  if(block_size - req_size >= MIN_BLOCK_SIZE){

    // Set this block tags to be the requested size and preserve other tags
    ptr_free_block->size_and_tags = req_size | TAG_USED | preceding_block_use_tag;

    // Assign a pointer to the block left over after the allocation
    ptr_next_block = ((block_info*)UNSCALED_POINTER_ADD(ptr_free_block, req_size));

    // Set the following split block's header tag to reflect the new, smaller size and the preceding block's use
    ptr_next_block->size_and_tags = ((block_size - req_size) | TAG_PRECEDING_USED);

    // Set the new block's footer tag to be equivalent to the header
    ((block_info*)UNSCALED_POINTER_ADD(ptr_free_block, block_size - WORD_SIZE))->size_and_tags = ptr_next_block->size_and_tags;

    // Insert the new split block into the free list
    insert_free_block(ptr_next_block);

    //Return a pointer to the start of the allocated block's payload
    return UNSCALED_POINTER_ADD(ptr_free_block, WORD_SIZE);
  }
  //If the left over space does not meet the minimum block length...
  else{

    // Access left over space's header and update it
    ptr_next_block = ((block_info*) UNSCALED_POINTER_ADD(ptr_free_block, block_size));
...
void mm_free(void* ptr) {
  ...
  // Check if the pointer passed in is valid before proceeding.
  if(ptr != NULL){
    // Get BlockInfo, size, tags and following block, of passed in ptr (assuming it points to payload).
    block_to_free = UNSCALED_POINTER_SUB(ptr, WORD_SIZE);

    // Mark valid bit as invalid by flipping it with xor
    block_to_free->size_and_tags = (block_to_free->size_and_tags)^TAG_USED;

    // Update prev and next in consecutive block
    insert_free_block(block_to_free);

    // Instantiate a pointer to reference the next block
    following_block = ((block_info*)UNSCALED_POINTER_ADD(block_to_free, SIZE(block_to_free->size_and_tags) + WORD_SIZE));

    // Coalesce if following block is free or if preceding_used bit is 0 (indicating previous block is also free)
    if(((following_block->size_and_tags) & TAG_USED) == 0 || ((block_to_free->size_and_tags) & TAG_PRECEDING_USED) == 0){
